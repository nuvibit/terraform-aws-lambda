...
| <a name="input_enable_encryption"></a> [enable\_encryption](#input\_enable\_encryption) | Actually a redundancy to kms\_key\_arn == null. <br>  But required to cope with terraform plan issues when an external provided KMS CMK is created<br>  Set to true to enable encryption of logs and sqs messages. Requires kms\_key\_arn to be set. | `bool` | `false` | no |
