...
| <a name="input_triggering_sns_topics"></a> [triggering\_sns\_topics](#input\_triggering\_sns\_topics) | List of SNS ARNs for triggering the Lambda. If provided a SQS for triggering the Lambda will be added and subscribed to the SNS. | <pre>list(object(<br>    {<br>      sns_arn            = string # The ARN of the SNS topic that will trigger the Lambda via the created SQS.<br>      filter_policy_json = string # Policy for filtering the stream to the SQS subscription to specific SNS items<br>    }<br>  ))</pre> | `[]` | no |
